...
def firstLogin():
    ...
    masterPasswordLabel = Label(
        loginWindow, text="Set master password", fg=colorWhite, bg=backgroundColor)
    masterPasswordLabel.config(anchor=CENTER)
    masterPasswordLabel.pack()

    masterPasswordEntry = Entry(
        loginWindow, width=20, show="*", bg=entryBackgroundColor, fg=colorBlack)
    masterPasswordEntry.pack()
    masterPasswordEntry.focus()

    masterPasswordConfirmLabel = Label(
        loginWindow, text="Confirm master password", fg=colorWhite, bg=backgroundColor)
    masterPasswordConfirmLabel.pack()

    masterPasswordConfirmEntry = Entry(
        loginWindow, show="*", bg=entryBackgroundColor, fg=colorBlack)
    ...
    saveMasterPasswordButton = Button(
        loginWindow, text="Save master password", command=saveMasterPassword, fg="black", bg=backgroundColor)
...
def regularLogin():
    ...
    loginLabel = Label(loginWindow, text="Enter master password",
                       fg=colorWhite, bg=backgroundColor)
    loginLabel.config(anchor=CENTER)
    loginLabel.pack()

    loginEntry = Entry(loginWindow, show="*", bg=entryBackgroundColor,
                       fg=colorBlack, insertbackground="black")
    loginEntry.pack()
    loginEntry.focus()
    ...
    def getMasterPasswordFromDB():
        ...
        cursor.execute(
            "SELECT * FROM masterpassword WHERE id = 1 AND password = ?", [checkHashedMasterPassword])
    ...
    loginWithMasterPasswordButton = Button(
        loginWindow, text="log in", command=checkMasterPassword, fg=colorBlack, bg=backgroundColor)
...
def startPasswordManager():
    ...
    mainWindow.tk.call('wm', 'iconphoto', mainWindow._w,
                       PhotoImage(file='images/icons/lock.png'))
    ...
    def passwordGeneratorWindow():
        ...
        passwordGeneratorLabel = Label(
            passGen, text="Password: ", fg=colorWhite, bg=backgroundColor)
        passwordGeneratorLabel.grid(row=0, padx=10)
        generatedPasswordField = Entry(
            passGen, state=DISABLED, bg=entryBackgroundColor, fg="white")
        ...
        Checkbutton(passGen, text="Non-capital letters (abc)", variable=varNonCapitalLetters, command=checkPasswordGeneratorOptions,
                    fg=colorWhite, bg=backgroundColor, selectcolor=buttonBackgroundColor).grid(row=2, column=0)
        Checkbutton(passGen, text="Capitals letters (ABC)", variable=varCapitalLetters, command=checkPasswordGeneratorOptions,
                    fg=colorWhite, bg=backgroundColor, selectcolor=buttonBackgroundColor).grid(sticky="w", row=3, column=0)
        Checkbutton(passGen, text="Numbers (123)", variable=varNumbers, command=checkPasswordGeneratorOptions,
                    fg=colorWhite, bg=backgroundColor, selectcolor=buttonBackgroundColor).grid(row=2, column=1)
        Checkbutton(passGen, text="Symbols (#$&)", variable=varSymbols, command=checkPasswordGeneratorOptions,
                    fg=colorWhite, bg=backgroundColor, selectcolor=buttonBackgroundColor).grid(row=3, column=1)

        # Creates label to display the text for the password length
        generatedPasswordLengthLabel = Label(
            passGen, text="Password length:", fg=colorWhite, bg=backgroundColor)
        generatedPasswordLengthLabel.grid(row=1, column=0, padx=10)

        # Creates scale from 8 to 30 for selecting the password length
        passwordLengthScale = Scale(passGen, from_=8, to=30, orient=HORIZONTAL, command=gebeLaengeAn, fg=colorWhite,
                                    bg=backgroundColor, activebackground=backgroundColor, highlightbackground=backgroundColor)
        ...
        passwordTip = Label(
            passwordTipFrame, text="Make sure to use \ndifferent passwords \nfor different accounts!", fg=colorWhite, bg=backgroundColor)
        passwordTip.grid(row=0, column=0, rowspan=3)

        # Creates button for copying the password to the clipboard
        copyPasswordButton = Button(
            passGen, text="Copy password", command=copyGeneratedPassword, fg=colorBlack, bg=backgroundColor)
        ...
        generatePasswordButton = Button(
            passGen, text="Generate password", command=createPassword, fg=colorBlack, bg=backgroundColor)
        generatePasswordButton.grid(row=6, column=0)

        # Creates button for canceling password creation
        cancelPasswordCreationButton = Button(
            passGen, text="Cancel", command=cancelPasswordGenerator, fg=colorBlack, bg=backgroundColor)
        cancelPasswordCreationButton.grid(row=6, column=2)

        # Creates button for confirming the password
        confirmPasswordButton = Button(
            passGen, text="Confirm password", command=confirmPassword, fg=colorBlack, bg=backgroundColor)
    ...
    def deleteRecordFromDatabase():
        ...
        if(deleteRecordID != ""):
            ...
            try:
                ...
                cursor.execute(
                    "Select appName from pwManager where oid = " + deleteRecordID)
                ...
                # Deletes the record if it is found in the database
                    ...
                    nameEntryField.delete(0, END)
                    cursor.execute(
                        "DELETE FROM pwManager where oid = " + deleteRecordID)
                    messagebox.showinfo(
                        "Information", "Deleted record %s!" % deleteRecordID)
                    launchPasswordGenerator.config(state="disabled")
    ...
    def hidePasswordRecords():
        ...
        showRecordsButton.config(
            text="Show records", command=lambda: showRecords(3))
    ...
    def updatePasswordRecord():
        ...
        if nameEntryField.get() != "" or urlEntryField.get() != "" or usernameEntryField.get() != "" or passwordEntryField.get() != "":
            ...
            if(checksum != 0):
                ...
                dbFrame.destroy()
                showRecords(3)
                addRecordButton.config(
                    text="Add record", command=addRecordToDatabase)
                showRecordsButton.config(
                    text="Hide records", command=hidePasswordRecords)
                deleteRecordEntryField.config(state=NORMAL)
                editRecordEntryField.config(state=NORMAL)
    ...
    def preUpdateRecord():
        ...
        if(updateRecordID != ""):
            ...
            try:
                cursor.execute(
                    "Select appName from pwManager where oid = " + updateRecordID)
                ...
                # Outputs the saved values if the record exists in the database
                    ...
                    addRecordButton.config(
                        text="Update record", command=updatePasswordRecord)
                    showRecordsButton.config(
                        text="Cancel", command=cancelUpdatingRecord)
                    deleteRecordEntryField.config(state=DISABLED)
                    editRecordEntryField.config(state=DISABLED)

                    cursor.execute(
                        "Select url from pwManager where oid = " + updateRecordID)
                    ...
                    cursor.execute(
                        "Select username from pwManager where oid = " + updateRecordID)
                    record = cursor.fetchall()
                    usernameEntryField.delete(0, END)
                    usernameEntryField.insert(END, record)

                    cursor.execute(
                        "Select password from pwManager where oid = " + updateRecordID)
                    record = cursor.fetchall()
                    passwordEntryField.config(state=NORMAL)
                    passwordEntryField.delete(0, END)
                    ...
                    if("{{" in removeBracketsInPassword):
                        ...
                        removeBracketsInPassword = removeBracketsInPassword.replace(
                            "}}", "")
                        passwordEntryField.insert(
                            END, removeBracketsInPassword)
    ...
    def createEyeButtons():
        ...
        showHidePWsButton = Button(
            mainWindow, image=photo, command=showFirstRecords, bg="blue")
    ...
    def showRecords(x):
        ...
        try:
            ...
            pass
        dbFrame = Frame(
            mainWindow, highlightbackground="#3a3b3c", highlightthickness=4)
        dbFrame.place(relx=0.51, rely=0.5, relwidth=0.99,
        ...
        if(x == 1):
            ...
            deleteRecordButton = Button(
                mainWindow, text="Delete record", command=deleteRecordFromDatabase, fg=colorBlack, bg=backgroundColor)
            deleteRecordButton.grid(row=7, column=0)
            global deleteRecordEntryField
            deleteRecordEntryField = Entry(
                mainWindow, fg=colorBlack, bg=entryBackgroundColor, insertbackground="black")
            ...
            updateRecordButton = Button(
                mainWindow, text="Edit record", command=preUpdateRecord, fg=colorBlack, bg=backgroundColor)
            updateRecordButton.grid(row=8, column=0)
            global editRecordEntryField
            editRecordEntryField = Entry(
                mainWindow, fg=colorBlack, bg=entryBackgroundColor, insertbackground="black")
            ...
            def removePlaceholderInDeleteEntry(event):
                deleteRecordEntryField.delete(0, END)
                deleteRecordEntryField.unbind(
                    '<Button-1>', removePlaceHolderInDeleteEntryBind)
            removePlaceHolderInDeleteEntryBind = deleteRecordEntryField.bind(
                '<Button-1>', removePlaceholderInDeleteEntry)
            ...
            def removePlaceholderInUpdateEntry(event):
                editRecordEntryField.delete(0, END)
                editRecordEntryField.unbind(
                    '<Button-1>', removePlaceHolderInUpdateEntryBind)
            removePlaceHolderInUpdateEntryBind = editRecordEntryField.bind(
                '<Button-1>', removePlaceholderInUpdateEntry)
    ...
    nameEntryField = Entry(mainWindow, width=25, fg=colorBlack,
                           bg=entryBackgroundColor, insertbackground="black")
    nameEntryField.grid(row=0, column=1, padx=25, pady=5)
    usernameEntryField = Entry(mainWindow, width=25, fg=colorBlack,
                               bg=entryBackgroundColor, insertbackground="black")
    usernameEntryField.grid(row=1, column=1, padx=20, pady=5)
    urlEntryField = Entry(mainWindow, width=25, fg=colorBlack,
                          bg=entryBackgroundColor, insertbackground="black")
    urlEntryField.grid(row=2, column=1, padx=20, pady=5)
    passwordEntryField = Entry(mainWindow, width=25, show="*",
                               fg=colorBlack, bg=entryBackgroundColor, insertbackground="black")
    passwordEntryField.grid(row=4, column=1, padx=20, pady=5)

    # Creates the labels for the password parameters
    nameEingabeLabel = Label(mainWindow, text="Name:",
                             fg=colorWhite, bg=backgroundColor)
    nameEingabeLabel.grid(row=0, column=0)
    userNameEntryLabel = Label(
        mainWindow, text="Username:", fg=colorWhite, bg=backgroundColor)
    userNameEntryLabel.grid(row=1, column=0)
    urlEntryLabel = Label(mainWindow, text="URL (optional):",
                          fg=colorWhite, bg=backgroundColor)
    urlEntryLabel.grid(row=2, column=0)
    passwordEntryLabel = Label(
        mainWindow, text="Password:", fg=colorWhite, bg=backgroundColor)
    passwordEntryLabel.grid(row=4, column=0)

    # Creates the radiobuttons
    getRadiobuttonValue = StringVar()
    manualRadiobutton = Radiobutton(mainWindow, text="Manual password", variable=getRadiobuttonValue, value="M",
                                    command=checkPasswordManualGenerated, fg=colorWhite, bg=backgroundColor, selectcolor=buttonBackgroundColor)
    manualRadiobutton.grid(row=3, column=1, sticky="w", padx=18)
    automaticRadiobutton = Radiobutton(mainWindow, text="Generate", variable=getRadiobuttonValue, value="A",
                                       command=checkPasswordManualGenerated, fg=colorWhite, bg=backgroundColor, selectcolor=buttonBackgroundColor)
    ...
    launchPasswordGenerator = Button(mainWindow, text="Generate password", command=passwordGeneratorWindow,
                                     state=DISABLED, fg=colorBlack, bg=colorWhite, disabledforeground=colorBlack)
    launchPasswordGenerator.grid(row=3, column=0)

    # Creates the button to add a record to the database
    addRecordButton = Button(mainWindow, text="Add record",
                             command=addRecordToDatabase, fg=colorBlack, bg=backgroundColor)
    addRecordButton.grid(row=5, column=0, padx=10, pady=20)

    # Creates the button to list the records from the database
    showRecordsButton = Button(mainWindow, text="Show records", command=lambda: showRecords(
        3), fg=colorBlack, bg=backgroundColor)
